...
def get_tranche(data:dict, type_only=True):
    tranche = {}
...
def filter_arrays_from_dict(d:dict):
    return {k:v for k,v in d.items() if isinstance(v, np.ndarray)}
def filter_lists_from_dict(d:dict):
    return {k:v for k,v in d.items() if isinstance(v, list)}

def get_pickle_data(pickle_files:list[Path], gdir, type_only=True):
    pickle_data = {}
    for pickle in pickle_files:
        try:
            ...
            if isinstance(stem, list):
                ...
                pickle_data[pickle.stem] = get_tranche(i, type_only=type_only)
                print(pickle_data[pickle.stem])
            else:
    ...
    for name, pickle in pickle_data.items():
        try:
            ...
            if isinstance(data, dict):
                data_tree = add_datacube(data_tree=data_tree, datacubes=data, datacube_name=name, overwrite=True)
        except TypeError as e:
            print(e)
    return data_tree

def write_zarr(
    data_tree, storage_directory: str, overwrite: bool = True, zarr_format: int = 2, encoding: dict = None
) -> None:
